...
#______________________________________________________________________________
# Queues: Stack, FIFOQueue, PriorityQueue, PriorityHQueue

class Queue:
    """Queue is an abstract class/interface. There are three types:
        Stack(): A Last In First Out Queue.
        FIFOQueue(): A First In First Out Queue.
        PriorityQueue: Queue where items are sorted by cost.
        PriorityHQueue: Queue where items are sorted by cost + heuristic.
    Cada tipo debe soportar:
        append(item, problem)
    ...
    def extend(self, items, problem):
        for item in items:
            self.append(item, problem)


class Stack():
    """Pila LIFO (para profundidad)"""
...
class PriorityQueue(Queue):
    ...
    def extend(self, items, problem):
        for it in items:
            self.append(it, problem)

    def pop(self):
        if not self.data:
            raise IndexError("pop from empty PriorityQueue")
        _, _, item = self.data.pop(0)
        return item
...
class PriorityHQueue(Queue):
    ...
    def pop(self):
        if not self.data:
            raise IndexError("pop from empty PriorityHQueue")
        _, _, item = self.data.pop(0)
        return item
