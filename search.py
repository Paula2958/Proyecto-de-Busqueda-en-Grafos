...
###PARTE A MODIFICAR POR MI


## Uninformed Search algorithms (y B&B)
...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            # OJO: el coste total ya está en node.path_cost
            return node, generated, visited

        # Solo expandimos si el estado no se ha cerrado aún
        if node.state not in closed:
            closed.add(node.state)

            successors = node.expand(problem)   # hijos (nuevos nodos)
            generated += len(successors)

            # La cola decide el orden: FIFO, LIFO, por coste, por coste+h...
            fringe.extend(successors, problem)
...
def branch_and_bound_graph_search(problem):
    """Ramificación y acotación (coste uniforme)."""
    return graph_search(problem, PriorityQueue())


def branch_and_bound_with_underestimation_graph_search(problem):
    """Ramificación y acotación con subestimación (g + h)."""
    return graph_search(problem, PriorityHQueue())
...
class GPSProblem(Problem):
    ...
    def h(self, node):
        """h = distancia en línea recta al objetivo."""
        locs = getattr(self.graph, 'locations', None)
        if locs:
            (ax, ay) = locs[node.state]
            (bx, by) = locs[self.goal]
            # Distancia euclídea, usando la distance de utils
            return int(distance((ax, ay), (bx, by)))
        else:
            return infinity
